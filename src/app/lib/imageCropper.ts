...
export class ImageCropper  {
  ...
  constructor(options) { 
    this.Cropper(options)// => undefined
  }
  ngOnInit() {
    // console.log(this.options) // => 'joining'
  }
...
Cropper(options) {
  ...
  // zoomIn: ' <button class="btn  mr-sm" type="button">+</button> ',
  //     zoomOut: ' <button class="btn  mr-sm" type="button">-</button> ',
  //     fit: '(fit)',
  //     crop: '<button class="btn  mr-sm" type="button">Crop</button>'
...
buildDOM() {
  ...
  _elements.controls.rotateLeft = this._buildControl(this.options.actionLabels.rotateLeft, 'rotateLeft');
  _elements.controls.rotateRight = this._buildControl(this.options.actionLabels.rotateRight, 'rotateRight');
  _elements.controls.zoomIn = this._buildControl(this.options.actionLabels.zoomIn, 'zoomIn');
  _elements.controls.zoomOut = this._buildControl(this.options.actionLabels.zoomOut, 'zoomOut');
  _elements.controls.fit = this._buildControl(this.options.actionLabels.fit, 'fit');

  _elements.controls.crop = this._buildControl(this.options.actionLabels.crop, 'crop');
 ...
 * @returns {Element}
 */
_buildControl(label, type) {
  var control = document.createElement('button');
  control.setAttribute('type', 'button');
  control.classList.add("btn");
  control.classList.add("m");  
  control.classList.add("box-shadow-btn");
  control.setAttribute("id", type);
...
setDimensions() {
  ...
  this.elements.wrapper.style.height = '319px';
...
initializeGesture() {
  ...
  var bind = function() {
    ...
    self.addEventListeners(self.elements.body, self.gesture.events.move, drag, false);
    self.addEventListeners(self.elements.body, self.gesture.events.stop, unbind, false);
  };

  var unbind = function() {
    self.elements.body.classList.remove('imgCropper-dragging');
    self.removeEventListeners(self.elements.body, self.gesture.events.move, drag, false);
    self.removeEventListeners(self.elements.body, self.gesture.events.stop, unbind, false);
  };
...
cropHandler() {
  ...
  this.options['croppedImage']=base64;
  return base64;
};

useHardwareAccelerate(element) {
 ...
 * @param context
 */
addEventListeners(element, eventNames, func, context) {
 ...
 * @param context
 */
removeEventListeners(element, eventNames, func, context) {
  eventNames.split(' ').forEach(function(eventName) {
    if (context) {
